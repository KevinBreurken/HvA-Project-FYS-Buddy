...
async function sendFriendEmail(idOfRecipient, textPrefix) {
    ...
    const recipientSetting = await getDataByPromise(`SELECT *
                                                     FROM setting
                                                     WHERE userId = ?`, idOfRecipient);
    ...
    const userLanguage = await getDataByPromise(`SELECT *
                                                 FROM language
                                                 WHERE id = ?`, recipientSetting[0].languageId);

    const url = window.location.href;
    getDataByPromise(`SELECT *
                      FROM user
                      WHERE id = ?`, idOfRecipient).then((userData) => {
        getDataByPromise(`SELECT *
                          FROM profile
